...
from src.pydantic.schemas import (
    Answer,
    Game,
    GameOut,
    JoinGame,
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            Question,
            dict(pk=0, topic=0, text="text", options=["a", "s", "d"], answer=0),
            None,
        ),
        (
            Player,
            dict(sid="sid", name="name"),
            dict(sid="sid", name="name", score=0, current_question=0, question_count=0),
        ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            Question,
            dict(pk=-1, topic=0, text="text", options=["a", "s", "d"], answer=0),
        ),
        (
            Question,
            dict(pk=0, topic=-1, text="text", options=["a", "s", "d"], answer=0),
        ),
        ...
        (
            Question,
            dict(pk=0, topic=0, text="text", options=["a", "s", "d"], answer=-1),
        ),
        #
